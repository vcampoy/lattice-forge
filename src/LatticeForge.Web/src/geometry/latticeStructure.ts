...
export function calculateLatticeInstances(parameters: BracketGeometryParameters & { latticeDensity: number }, maxInstances = LATTICE_MAX_INSTANCES): LatticeInstance[] {
  ...
  const instanceLimit = Math.min(LATTICE_MAX_INSTANCES, Math.max(1, Math.floor(maxInstances)))
  for (let layer = 0; layer < zLayers && instances.length < instanceLimit; layer += 1) {
    const z = zLayers === 1 ? 0 : interpolate(minZ, maxZ, layer, zLayers - 1)
    for (let row = 0; row < ySegments && instances.length < instanceLimit; row += 1) {
      const y0 = interpolate(minY, maxY, row, ySegments)
      const y1 = interpolate(minY, maxY, row + 1, ySegments)
      for (let column = 0; column < xSegments && instances.length < instanceLimit; column += 1) {
